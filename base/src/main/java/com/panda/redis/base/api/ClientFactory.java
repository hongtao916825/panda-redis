...
public class ClientFactory implements PooledObjectFactory<Client> {
  ...
  ClientFactory(final String host, final int port, final int connectionTimeout,
  ...
  ClientFactory(final URI uri, final int connectionTimeout, final int soTimeout,
  ...
  @Override
  public void activateObject(PooledObject<Client> pooledJedis) throws Exception {
  ...
  @Override
  public void destroyObject(PooledObject<Client> pooledJedis) throws Exception {
    final BinaryJedis jedis = pooledJedis.getObject();
  ...
  public PooledObject<Client> makeObject() throws Exception {
    ...
    final Client jedis = new Client(hp.getHost(), hp.getPort(), connectionTimeout, soTimeout,
        ssl, sslSocketFactory, sslParameters, hostnameVerifier);
    try {
      jedis.connect();
      if (user != null) {
  ...
  @Override
  public void passivateObject(PooledObject<Client> pooledJedis) throws Exception {
    // TODO maybe should select db 0? Not sure right now.
  }

  @Override
  public boolean validateObject(PooledObject<Client> pooledJedis) {
    final BinaryJedis jedis = pooledJedis.getObject();
    try {
